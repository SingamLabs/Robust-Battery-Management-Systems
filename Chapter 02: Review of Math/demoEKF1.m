...
for k = 2:n
    [xkk, Pkk, NIS(k)] = EKF(xkk, Pkk, Q, R, zk(k), k); 
...
plot(zk, 'Color',[0 0.7 0], 'linewidth', 2,'LineStyle','none','Marker','*','MarkerSize',10)
plot(xekf, 'Color',[1 0.2 0.8], 'linewidth', 2)

%Plot labels
xlabel('Time index, k', 'FontSize', 15) 
legend({'True value $x(k|k)$','Measurement $z (k)$', ...
    'Estimate $\hat x(k|k)$ '},'interpreter', 'latex' ) 

%saving figure
filename = '../_figures/EKFest';
...
% Plotting estimate
h=figure; hold on; box on; grid on
line(1:length(NIS), r1*ones(size(NIS)),'Linestyle','--', 'Linewidth',2,'Color',[1 0 0])
line(1:length(NIS), r2*ones(size(NIS)),'Linestyle','--', 'Linewidth',2,'Color',[1 0 0])
